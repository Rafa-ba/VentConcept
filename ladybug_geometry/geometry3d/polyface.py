...
class Polyface3D(Base2DIn3D):
    ...
    @classmethod
    def from_faces_index(cls, faces, index, tolerance):
        """Initialize Polyface3D from a list of Face3D objects.
        ...
        for i in range(index):
            ind = []
            loops = (faces[i].boundary,) if not faces[i].has_holes else (faces[i].boundary,) + faces[i].holes
            for j, loop in enumerate(loops):
                ind.append([])
                for v in loop:
                    found = False
    ...
    def get_outward_faces(faces, tolerance):
        ...
        for i, face in enumerate(faces):
            ...
            if n_int % 2 == 0:
                ...
                print("huuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuu")
                already_added = False
                for _f in faces[i + 1:]:
                    if _f.plane.is_coplanar_tolerance(face.plane, tolerance, 0) and _f.normal.is_equivalent(face.normal, tolerance):
                        outward_faces.append(face)
                        already_added = True
                        print("HAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")

                for _f in faces[:i]:
                    if _f.plane.is_coplanar_tolerance(face.plane, tolerance, 0) and _f.normal.is_equivalent(face.normal, tolerance):
                        outward_faces.append(face)
                        already_added = True
                        print("HAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")
